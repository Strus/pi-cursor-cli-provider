...
function formatToolResultLines(
	...
	if (toolName === "bash") {
		...
		if (exitCode != null && exitCode !== 0) {
			statusLines.push(theme.fg("error", `[exit ${exitCode}]`));
		}
		if (remaining > 0) {
	...
	return { isError: false, lines: [theme.fg("muted", "Completed.")] };
}
...
function renderToolBlock(
	...
	return `\n${lines.map((line) => theme.bg(bg, ` ${line || " "} `)).join("\n")}\n`;
}

export function renderStartedToolCall(
	cliKey: string,
	payload: CursorToolCallPayload,
): string {
	const toolName = toPiToolName(cliKey);
	const title = formatToolCallTitle(toolName, payload.args ?? {});
	return renderToolBlock("toolPendingBg", title);
}
